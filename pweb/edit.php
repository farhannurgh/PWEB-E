...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "UPDATE pendaftaran SET nama='$nama', alamat='$alamat', no_hp='$no_hp', kursus='$kursus', status='$status' WHERE id=$id";
    if ($conn->query($sql)) {
...
<body>
    ...
    <form method="POST">
        ...
        <select name="status">
            ...
            <option value="Pelajar" <?= $row['status'] === 'Pelajar' ? 'selected' : '' ?>>Pelajar</option>
        </select>
